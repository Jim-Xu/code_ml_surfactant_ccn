...
arm_train_data = pd.read_csv('/data/keeling/a/xx24/e/proj_ml/code_ml_surfactant_ccn/data/setpoint_0.2_train.csv') # e.g. Use 50% fine-tuning training dataset to fine-tune foundation model
arm_test_data = pd.read_csv('/data/keeling/a/xx24/e/proj_ml/code_ml_surfactant_ccn/data/setpoint_0.2_test.csv')
...
def objective(trial):
    print(f"正在进行第 {trial.number + 1} 次 Trial")
    ...
    num_frozen_blocks = trial.suggest_int("num_frozen_blocks", 15, 18)  # Optimize number of frozen layers
    ...
    model.load_state_dict(torch.load('/data/keeling/a/xx24/e/proj_ml/code_ml_surfactant_ccn/model/Foundation_Model_0.2.pth'))
